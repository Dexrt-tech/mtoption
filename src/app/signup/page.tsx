...
import { Eye, EyeOff, ArrowRight, Mail, Lock, User, Phone, TrendingUp, Star, AlertCircle, Shield } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to Meta Trading Option!');
  ...
  const inputClass = 'input-field h-11 pl-10 transition-all duration-200 focus:scale-[1.02]';
        ...
        <div className="mx-auto w-full max-w-[26rem]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              <User size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#62748e' }} />
              <input type="text" className={inputClass} placeholder="First Name" value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} required />
            </div>

            {/* Last Name */}
            <div className="relative">
              <User size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#62748e' }} />
              <input type="text" className={inputClass} placeholder="Last Name" value={form.lastName} onChange={(e) => setForm({ ...form, lastName: e.target.value })} required />
            </div>
            ...
            {/* Phone */}
            <div className="relative">
              <Phone size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#62748e' }} />
              <input type="tel" className={inputClass} placeholder="Phone Number" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
            </div>

            {/* Password */}
            <div className="relative">
              <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#62748e' }} />
              <input type={showPw ? 'text' : 'password'} className={`${inputClass} pr-10`} placeholder="Password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} minLength={8} required />
            ...
            <div className="relative">
              <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#62748e' }} />
              <input type="password" className={inputClass} placeholder="Confirm Password" value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })} required />
            </div>

            {/* Terms */}
            <label className="flex cursor-pointer items-start gap-3 text-xs" style={{ color: '#62748e' }}>
              <input type="checkbox" className="mt-0.5" style={{ accentColor: 'var(--primary)' }} checked={form.agreeTerms} onChange={(e) => setForm({ ...form, agreeTerms: e.target.checked })} />
              I agree to the{' '}
              <Link href="/terms-of-service" className="hover:underline" style={{ color: 'var(--primary)' }}>Terms of Service</Link>
              {' '}and{' '}
              <Link href="/privacy-policy" className="hover:underline" style={{ color: 'var(--primary)' }}>Privacy Policy</Link>
            </label>
